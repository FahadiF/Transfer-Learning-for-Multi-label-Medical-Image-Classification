...
def train_one_backbone(backbone, train_csv, val_csv, test_csv, train_image_dir, val_image_dir, test_image_dir, 
    ...
    for name, param in model.named_parameters():
        if backbone == "resnet18" and name.startswith('fc'):
            param.requires_grad = True
        elif backbone == "efficientnet" and name.startswith('classifier'):
            param.requires_grad = True
        else:
...
if __name__ == "__main__":
    ...
    test_image_dir = "./images/onsite_test" # replace with your own test image floder path
